...
Code 3 is separate for the same reason. A missing library or a missing data file
is the machine's fault, and reporting it as code 1 sends a student hunting for a
bug in code that is correct.
"""
...
def not_ready(problem) -> None:
    ...
    A check imports pandas, numpy or scipy at the top of the file, before run()
    can wrap anything in a try. Guard those imports with this and a student on an
    unprepared machine is told to run setup.sh, rather than being told the lab
    they have not opened yet is wrong.
    """
...
def run(lab_number: int, module_name: str, function_name: str, body, requires=()):
    ...
    earlier ones, so an unwritten Lab 2 would otherwise surface as "Lab 3 not
    written yet" quoting a message about Lab 2's function — which reads as a bug
    in the checks rather than as an order of work. Module 4 shipped exactly that:
    with Lab 2 unwritten, check 3 named Lab 3's own file and Lab 2's message.
    Probing first lets the message name both labs and still exit 2, which is the
    code for "not written yet".

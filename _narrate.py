...
def reference_lab(number: int):
    """Import a shipped solution by lab number, wired to reach the other solutions.

    Labs 3 and 4 reach into the labs before them through `lab_support.load_lab`,
    which reads `labs/` -- whatever the student has written. That is what makes
    the check a check, and it is also why a solution's demonstration cannot use
    it: `make demo` has to run before anybody has written `labs/02_*.py`.

    So the demonstrations rebind `load_lab` to this for the length of the run.
    It loads `solutions/lab_0N.py`, and puts itself into the loaded module so
    that a solution reaching for a solution keeps reaching for solutions. The
    checks never import this file.
    """
    import importlib.util

    path = HERE / "solutions" / f"lab_{number:02d}.py"
    specification = importlib.util.spec_from_file_location(f"reference_lab{number}", path)
    module = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(module)
    module.load_lab = reference_lab
    return module
